    ...
    public class FFMPEGInfo
    {
        ...
        public FFMPEGInfo(string filepath)
        {
            try
            {
                ...
                {
                    psImage.StartInfo.FileName = "ffmpeg64 ";
                }
                else
                {
                    psImage.StartInfo.FileName = "ffmpeg32 ";
                }
                ...
                psImage.StartInfo.Arguments = " -i \"" + filepath + "\"";

                if (psImage.StartInfo.Arguments.ToLower().EndsWith(".m2ts"))
                {
                    int a = 1;
                }
                ...
                string str1 = psImage.StandardError.ReadToEnd();
                ...
                while ((line = sr.ReadLine()) != null)
                {
                    ...
                    {
                        int bpos = line.IndexOf(", bitrate: ");

                        if (bpos >= 0)
                        {
                            bpos += ", bitrate: ".Length;

                            string bval = line.Substring(bpos).Replace(" kb/s", "k");

                            if (AudioBitRate.Trim() == string.Empty)
                            {
                                AudioBitRate = bval;
                            }

                            if (VideoBitRate.Trim() == string.Empty)
                            {
                                VideoBitRate = bval;
                            }
                        }
